...
class Creature:
  def __init__(self, levelset, position, stats):
  ...
  def goUp(self):
    ...
    self.pos['z'] -= 1
    log(4, chars[self.getChar()]+' went up a dungeon level. z', self.pos['z'])
    self.loadLevel(DOWN_STAIRS_ID)
    return True
  def goDown(self):
    if self.pos['z'] >= self.levelset.getHeight()-1: return False
    self.pos['z'] += 1
    log(4, chars[self.getChar()]+' went down a dungeon level. z', self.pos['z'])
      ...
      for levelX, levelY in doublerange(level.getWL()):
        ...
        for roomX, roomY in doublerange(room.getWL()):
          ...
          if not tile.getTerrain() in solid and frequency > random.random():
            ...
            self.creatures.append( Monster(levelset,pos, Stats(2, 3)) )
            log(4, 'Added monster.')
...
#    log(5, 'y', y)
  ...
  for creature in reversed(creatures.getList()):
    ...
    roomWin.addch(pos['roomY'],pos['roomX'],chars[char], curses.color_pair(color))
...
def newGame(height, creatureFrequency, playerName):
  ...
  player = Player(levelset, levelset.getStartingPos(), Stats(20, 21))
...
# 	NEW FEATURES:
# catch KeyboardInterrupt exceptions and save/quit
# do the TODO's

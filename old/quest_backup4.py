...
#     if self.getName() != 'player': #uncomment to walk through walls (doesn't work now)
    ...
    if not enemy == None: #TODO return enemy instead of dealing with it here
...
class Creature:
  ...
  def wander(self, creatures):
    return self.location.move(random.randint(-1,1),random.randint(-1,1), creatures, self.getLevel())
...
#    log(2, 'dy', dy)
    ...
    else:self.location.move(dx, dy, creatures, self.getLevel())
    return True
...
#    log('======raycast at ('+str(startX)+', '+str(startY)+')=======')
  ...
  #returns whether or not the player moved/took an action
    ...
    dy = [ 1, 1, 1, 0, -1, -1, -1,  0]
    for i in range(8):
      if key == moveOpts[i][2]: return self.location.move(dx[i], dy[i], creatures, self.getLevel())

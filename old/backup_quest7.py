#!/usr/bin/python
import os, random, math, time, curses, sys, itertools, gzip, signal
...
class Tile:
  ...
  #TODO (possibly) make terrain an item and add item colors
...
def newCardinalsFalse(): return newCardinals(False, False, False, False)
def newCardinalsTrue(): return newCardinals(True, True, True, True)
...
class Player(Creature):
  ...
  def act(self, creatures, key): #returns whether or not the player moved/took an action
    level = self.getLevel()
    levelWidth, levelLength = level.getWL()
    room = self.getRoom()
    roomWidth, roomLength = room.getWL()
      ...
      for levelX, levelY in doublerange(level.getWL()):
        ...
        for roomX, roomY in doublerange(room.getWL()):
          ...
          if not tile.getTerrain() in solid and frequency > random.random():
            ...
            monster.setType(random.randrange(len(monsters)))
...
#    if len(key)==1: log(1, 'key code', ord(key))
    if   key=='KEY_LEFT': x-=1
      ...
      if x<length: text = text[:x]+text[x+1:]
...
def yesNoPrompt(screen, y, x, prompt, default=None):
  defaultStr=''
  if default==True:  defaultStr='[y]'
  if default==False: defaultStr='[n]'
  ...
  while input==None:
    ...
    if input==None: imput=default #if the user pressed something else, use the default if there is one
...
def saveGame(saveFilePath, screen, creatures):
  ...
  pickle.dump(creatures, file) #save the game
  file.close()
...
def loadGame(saveFilePath, screen):
  ...
  try:
    ...
    if yesNoPrompt(screen, 0, 0, "The save file is corrupt. Delete it?"):
      ...
      newPath = os.path.join(rootDir, os.path.basename(saveFilePath))
...
# 	DONE:
# move left and right in text fields

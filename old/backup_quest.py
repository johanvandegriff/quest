...
rootDir = os.path.join(os.path.expanduser('~'), '.quest')
saveGameDir = os.path.join(rootDir,'save')
...
#            log(1, 'Added monster. z:'+str(z)+' levelXY:('+str(levelX)+','+str(levelY)+') roomXY:('+str(roomX)+','+str(roomY)+')')
  ...
  def act(self):
    self.removeDead()
    playerZ = self.getPlayer().getLocation().getZ()
    ...
    #the copy also keeps it from messing up if one dies
      if creature.getLocation().getZ() != playerZ:continue #only creatures on the same level as the player get to act
#      if creature.getLocation().getZ() - playerZ <= 1:continue #only creatures near the same level as the player get to act
  ...
  def findAt(self, z, levelX, levelY, roomX, roomY):
    for creature in self.creatures:
      ...
      if location.getZ() != z:continue
      if location.getLevelXY() != (levelX, levelY):continue
      if location.getRoomXY() != (roomX, roomY):continue
      return creature
    return None
...
def extendedCommand(screen): return textField(screen, 0,0, '# ', 60)
...
def saveGame(saveFilePath, creatures):
  log(10, 'Saving...')
...
def newGame(height, creatureFrequency, playerName):
  startTime = time.time()
  #good layout: level:6x3 room:13x11 or 15x11
  levelset = randomLevelset(height)
  player = Player(levelset, levelset.getStartingLocation(), Stats(20, 21))
...
def main(screen):
  ...
  if playerName+'.save' in saveFiles:
    ...
    levelset, player, creatures = loadGame(saveFilePath)
  else:
    log(5, playerName+' does not have a save file.')
    levelset, player, creatures = newGame(50, 0.01, playerName) #TODO use multithreading to hide the create game lag
  while True:
    ...
    while not player.act(creatures, key):
      if key == 'S':
        saveGame(saveFilePath, creatures)
        return
      if key == 'o':
        optionsMenu(screen)
        display(screen, creatures)

...
def display(screen, levelset, player): #room, roomX, roomY, level, levelX, levelY):
...
def main(screen):
  ...
  player = Player(z, levelX, levelY, roomX, roomY)
  ...
  while True:
    ...
    while not moved:
      ...
      if   key == options[0][1]: dx=-1; dy= 1
        ...
        display(screen, room, roomX, roomY, level, levelX, levelY)
        screen.refresh()
      if not moved: moved = player.move(dx, dy, room)

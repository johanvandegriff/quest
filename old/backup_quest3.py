...
inverted = [] #displayed with highlighting
...
def randSkip(min, max):	return random.choice(list(range(min, max, 2)))
...
#TODO change doors to a hash table
...
class Monster(Creature):
  ...
  def act(self, creatures): #TODO monsters get stuck in doors
    ...
    Creature.act(self) #call the superclass act method
      ...
      for levelX, levelY in doublerange(level.getWL()):
        ...
        for roomX, roomY in doublerange(room.getWL()):
          ...
          if not tile.getTerrain() in solid and frequency > random.random():
            ...
            self.creatures.append( Monster(levelset, pos, 3) )
#            log(4, 'Added monster.')
...
#  screen.addstr(1,0,level.getName()+' HP:'+str(player.getHealth())+'/'+str(player.getMaxHealth())+' '+creatureStats, curses.A_REVERSE)
...
def newGame(screen, height, creatureFrequency, playerName):
  ...
  player = Player(levelset, levelset.getStartingPos(), 21)
...
def saveMenu(saveFilePath, screen, creatures):
  ...
  if screen.getkey() in ['y', 'Y']:
    screen.addstr(0,0,"                           ") #this clears the prompt message

...
def initializeChars():  #chars = '.#^v!Os-*:++;[???---////TT(r)?[[[]]]]?ooooo???????'
  global chars, colors
...
#    self.randomizeRooms()
  ...
  def addStairs(self): #adds both up and down stairs to the level
    log(4, "    min Stair Distance", (self.width**2+self.length**2)/5.0)
    while (self.upX-self.downX)**2+(self.upY-self.downY)**2 < (self.width**2+self.length**2)/5.0:
...
# 
# 	BUGS:
# monsters going downstairs immediately die
